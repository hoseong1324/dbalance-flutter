...
class RoomsRepository {
  ...
  Future<List<Room>> getRecentRooms() async {
    try {
      final response = await _dioClient.dio.get('/rooms/recent');
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('최근 방 목록 조회 실패: ${response.statusCode}');
      }
    } on DioException catch (e) {
      if (kDebugMode) {
        print('최근 방 목록 조회 오류: ${e.message}');
  ...
  Future<Room> createRoom({
    ...
    try {
      final response = await _dioClient.dio.post('/rooms', data: {
        'name': name,
  ...
  Future<Room> updateRoom(int roomId, {
    ...
    try {
      final response = await _dioClient.dio.patch('/rooms/$roomId', data: {
  ...
  Future<List<Round>> getRounds(int roomId) async {
    try {
      final response = await _dioClient.dio.get('/rooms/$roomId/rounds');

      if (response.statusCode == 200) {
        final data = response.data;
        if (data is Map<String, dynamic> && data.containsKey('data')) {
          final List<dynamic> rounds = data['data'];
          return rounds.map((json) => Round.fromJson(json)).toList();
        } else if (data is List) {
  ...
  Future<Round> createRound(int roomId, {
    ...
    try {
      final response = await _dioClient.dio.post('/rooms/$roomId/rounds', data: {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        final data = response.data;
        if (data is Map<String, dynamic> && data.containsKey('data')) {
          return Round.fromJson(data['data']);
        } else {
  ...
  Future<List<RoomParticipant>> addParticipants(int roomId, List<Map<String, dynamic>> participants) async {
    try {
      final response = await _dioClient.dio.post('/rooms/$roomId/participants', data: {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        final data = response.data;
        if (data is Map<String, dynamic> && data.containsKey('data')) {
          final List<dynamic> participants = data['data'];
          return participants.map((json) => RoomParticipant.fromJson(json)).toList();
        } else if (data is List) {
  ...
  Future<RoomParticipant> updateParticipant(int roomId, int participantId, {
    ...
    try {
      final response = await _dioClient.dio.patch('/rooms/$roomId/participants/$participantId', data: {
      ...
      if (response.statusCode == 200) {
        final data = response.data;
        if (data is Map<String, dynamic> && data.containsKey('data')) {
          return RoomParticipant.fromJson(data['data']);
        } else {
  ...
  Future<int> deleteParticipant(int roomId, int participantId) async {
    try {
      final response = await _dioClient.dio.delete('/rooms/$roomId/participants/$participantId');
